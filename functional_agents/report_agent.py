...
class ReportAgent(FunctionalAgent):
    ...
    def _execute(self, context: AgentContext) -> AgentContext:
        ...
        # PH2.1 – planner LLM boundary diagnostics (stages / normalization /
        # validation) — additive.
        _planner_boundary = context.trace.get("_planner_boundary")
        if _planner_boundary:
            trace_payload["planner_boundary"] = _planner_boundary
        ...
        # PH2.2 – evidence LLM boundary diagnostics (stages / normalization /
        # validation) — additive.
        _evidence_boundary = context.trace.get("_evidence_boundary")
        if _evidence_boundary:
            trace_payload["evidence_boundary"] = _evidence_boundary
        ...
        # PH2.3 – hypothesis LLM boundary diagnostics (additive).
        _hypothesis_boundary = context.trace.get("_hypothesis_boundary")
        if _hypothesis_boundary:
            trace_payload["hypothesis_boundary"] = _hypothesis_boundary
